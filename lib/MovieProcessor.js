...
module.exports = class MovieProcessor {
    ...
    __resolveRelativePath(relativePath) {
        ...
        if (relativePath) {
            if (relativePath.indexOf("\\") != -1 && os == 'linux') relativePath.replace("\\", "/");
            else if (relativePath.indexOf("/") != -1 && os == 'win32') relativePath.replace("/", "\\");
    ...
    async __createTempFile(fileName = "", fileType = ".bin", bufferedData = []) {
        return new Promise(async (resolve, reject) => {
            ...
            //fs.open(temp) ;
            console.log("\t[CREATE]: " + this.chalk.green(temp));
    ...
    async process(createResources = true) {
        console.log("\t[PROCESS]: " + this.chalk.yellow(this.moviePath));
        return new Promise((resolve, reject) => {
            this.movieInfo.getAllInfo(this.moviePath).then((json) => {
                if (json) {
                    if (json.original_title)
                        console.log("\t[TITLE]: " + this.chalk.cyan(json.original_title));
                    (async () => {
...
// var mov = new MovieProcessor("I:\\Movies\\S\\The Sunset Limited (2011)\\Avengers Endgame.mp4");
